  ...
  async function buscarRelatorio() {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/api/inactiveTables/consumptionReport?startDate=${inicio}&endDate=${fim}`,
              ...
              {dados.map((mesa) => (
                <tr key={mesa.tableId}>
                  ...
                  <td className="border p-2">
                    R$ {Number(mesa.totalConsumo).toFixed(2)}
                  </td>
                </tr>
              ))}
            </tbody>
